...
export function AdminPanel() {
  ...
  const handlePermissionToggle = async (
    ...
    try {
      ...
      setUsers((prevUsers) =>
        prevUsers.map((user) =>
          user._id === userId ? { ...user, permisos: newPermisos } : user
        )
      );
      ...
      setError(
        err instanceof Error ? err.message : "Error al actualizar permisos"
      );
    } finally {
      setSaving(null);
    }
  ...
  const handleCreateUser = async (userData: UserFormData) => {
    ...
    // Validar que la contraseña existe al crear usuario
    ...
    try {
      setError("");
      // Cast a CreateUserRequest ya que hemos validado que password existe
      await userService.createUser(userData as CreateUserRequest, token);
  ...
  const handleToggleStatus = async (user: Usuario) => {
    ...
    try {
      ...
      setSuccessMessage(
        `Usuario ${!user.activo ? "activado" : "desactivado"} correctamente`
      );
      setTimeout(() => setSuccessMessage(""), 3000);
        ...
        <div className="admin-header-content">
          <div className="admin-header-text">
            ...
            <p className="admin-subtitle">
              Gestiona los permisos de acceso a las aplicaciones
            </p>
          </div>
              ...
              return (
                <tr
                  ...
                  className={`${isSaving ? "saving" : ""} ${
                    !user.activo ? "inactive-user" : ""
                  }`}
                >
                  <td className="user-username">{nombreCompleto}</td>
                  <td className="status-cell">
                    <span
                      className={`status-badge ${
                        user.activo ? "status-active" : "status-inactive"
                      }`}
                    >
                    ...
                    <div className="action-buttons">
                      ...
                      >
                        ...
                        >
                          ...
                          />
                          <path
                            d="M19 12C19 12.3403 18.9508 12.6698 18.8593 12.9809C18.331 14.8486 16.3843 16 14 16H10C7.61575 16 5.66902 14.8486 5.14068 12.9809C5.04922 12.6698 5 12.3403 5 12C5 11.6597 5.04922 11.3302 5.14068 11.0191C5.66902 9.15142 7.61575 8 10 8H14C16.3843 8 18.331 9.15142 18.8593 11.0191C18.9508 11.3302 19 11.6597 19 12Z"
                            stroke="currentColor"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                      ...
                      <button
                        ...
                        className={`btn-action ${
                          user.activo ? "btn-deactivate" : "btn-activate"
                        }`}
                        title={
